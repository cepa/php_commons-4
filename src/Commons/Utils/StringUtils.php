...
/**
 * =============================================================================
 * @file       Commons/Utils/StringUtils.php
...
class StringUtils 
{
     ...
     * @return string
     */
    public static function numberWithCommas($number, $comma = ',')
    {
        $str = '';
        $number = (string) $number;
        for ($i = strlen($number), $j = 0; $i > 0; $i--, $j++) {
            if ($j > 0 && $j% 3 == 0) {
                $str = $comma.$str;
            }
            $str = $number{$i-1}.$str;
     ...
     * @return string
     */
    static public function getFilenameBase($filename)
    {
        $pos = strrpos($filename, '.');
            if ($pos === false)
                return $filename;
        return rtrim(substr($filename, 0, $pos), '.');
    }
    ...
    public static function fileSizeToText($size, $space = ' ', $round = 2)
    {
        ...
        $size /= 1024;    
        if ($size < 1024)
            return round($size, $round).$space.'kB';
        $size /= 1024;
        if ($size < 1024)
            return round($size, $round).$space.'MB';
        $size /= 1024;
        return round($size, $round).$space.'GB';
    }
    ...
    public static function latinNormalize($text, $separator = '-')
    {
        ...
        $norm = preg_replace('#[\-]+#', $separator, $norm);
        $norm = trim($norm, $separator);
        return $norm;
    }
    ...
    /**
     * Unaccent.
     * Originally from Doctrine_Inflector.
     * @param string $string
     * @return string
     */
