...
class EntityRepositoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testRepo_CompositeKey()
    {
        try {
            $database = \Bootstrap::createDatabase();
            $this->_connection
            ...
            $databaseType = $this->_connection->getDatabaseType();
            if ($databaseType == Sql::TYPE_MYSQL) {
                $this->executeStatement("CREATE TABLE test ( x int, y int, msg varchar(32), primary key (x, y) )");
            } else if ($databaseType == Sql::TYPE_POSTGRESQL) {
                $this->executeStatement("CREATE TABLE test ( x int, y int, msg varchar(32), primary key (x, y) )");
            } else {
                throw new Exception("Unsupported driver '{$databaseType}'!");
            ...
            $this->executeStatement("INSERT INTO test(x, y, msg) VALUES (1, 11, 'abc')");
            $this->executeStatement("INSERT INTO test(x, y, msg) VALUES (2, 22, 'def')");
            $this->executeStatement("INSERT INTO test(x, y, msg) VALUES (3, 33, 'ghi')");

            $repo = new EntityRepository($this->_connection);
            $repo->setPrimaryKey(array('x', 'y'));
            $this->assertNull($repo->getTableName());
            $r = $repo->setTableName('test');
            $this->assertTrue($r instanceof EntityRepository);
            $this->assertEquals('test', $repo->getTableName());
            ...
            $entity = $repo->fetch(array(1, 11));
            $this->assertTrue($entity instanceof Entity);
            $this->assertEquals(1, $entity->x);
            $this->assertEquals(11, $entity->y);
            $this->assertEquals('abc', $entity->msg);
            ...
            $this->assertEquals(1, $entity->x);
            $this->assertEquals(11, $entity->y);
            $this->assertEquals('xxx', $entity->msg);
            ...
            $this->assertNull($repo->fetch(array(1, 11)));
            ...
            $entity = $repo->fetch(array(4, 44));
            $this->assertTrue($entity instanceof Entity);
            $this->assertEquals(4, $entity->x);
            $this->assertEquals(44, $entity->y);
            $this->assertEquals('zzz', $entity->msg);
