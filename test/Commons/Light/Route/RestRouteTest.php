...
class RestRouteTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $route = new RestRoute('', 'user/(.*)', array(), array('user_id'));
        $params = $route->match($this->createRequest('GET', 'user/123'));
        $this->assertTrue(is_array($params));
        $this->assertEquals(123, $params['user_id']);
        ...
        $this->assertTrue(is_array($route->match($this->createRequest('GET', 'user/123'))));
        $this->assertTrue(is_array($route->match($this->createRequest('POST', 'user/123'))));
        $this->assertTrue(is_array($route->match($this->createRequest('PUT', 'user/123'))));
        $this->assertTrue(is_array($route->match($this->createRequest('DELETE', 'user/123'))));
        $this->assertTrue(is_array($route->match($this->createRequest('HEAD', 'user/123'))));
        $this->assertTrue(is_array($route->match($this->createRequest('TRACE', 'user/123'))));
        $this->assertTrue(is_array($route->match($this->createRequest('OPTIONS', 'user/123'))));
    }
